<template>
	<div class="message-signing-page">
		...
		<!-- form -->
		<form class="msg-form" @submit.prevent="handleSendP2PKH">
			<!-- form header -->
			<div class="msg-form-header">
				<div class="msg-form-title">
					...
					<span>P2PKH Form</span>
				</div>
				<p class="msg-form-description">Enter the recipient address and amount you want to send</p>
			</div>
			<!-- address input -->
			<div class="form-item">
				<label>Address</label>
			...
			<div class="form-item">
				...
				<MyTextarea
					v-model="satoshis"
					placeholder="Please input amount in satoshis (e.g. 1000000)"
					:readonly="false"
					:copyable="true"
					:deletable="true"
				/>
			...
			<div class="form-item-btn-container">
				<button
					...
					@click.stop="handleSendP2PKH"
					type="button"
					:disabled="!isFormValid || isSubmitting"
				>
					<span v-if="isSubmitting">Sending...</span>
					<span v-else>Send P2PKH</span>
				</button>
			</div>
		</form>
		<!-- result -->
		<div class="msg-form msg-result">
			<div class="msg-form-header">
				<div class="msg-form-title">
					...
					<span>P2PKH Sent Result</span>
				</div>
			</div>
			<MyTextarea
				v-model="sendResult"
				placeholder="P2PKH sent result will be displayed here"
				:readonly="true"
				:copyable="true"
				:deletable="false"
			/>
...
const validateForm = (): boolean => {
	...
	if (!satoshis.value || !satoshis.value.trim()) {
		...
		const satoshisNum = Number(satoshis.value.trim());
		if (isNaN(satoshisNum) || satoshisNum <= 0 || !Number.isInteger(satoshisNum)) {
			errors.value.satoshis = 'Satoshis must be a positive integer';
			isValid = false;
		} else {
			errors.value.satoshis = '';
		}
