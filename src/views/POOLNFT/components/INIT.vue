	...
	<form class="msg-form" @submit.prevent="handleSubmit">
		...
		<div class="msg-form-header">
			<div class="msg-form-title">
				...
				<span>POOLNFT INIT Form</span>
			</div>
			<p class="msg-form-description">
				Configure the POOLNFT INIT parameters before submitting the transaction.
			</p>
		</div>
		...
		<div class="form-item-btn-container">
			<button class="form-button-submit" type="submit" :disabled="!isFormValid || isSubmitting">
				...
				<span v-else>Send POOLNFT INIT</span>
			</button>
		</div>
	</form>
	...
	<div class="msg-form msg-result">
		<div class="msg-form-header">
			<div class="msg-form-title">
				...
				<span>POOLNFT INIT Result</span>
			</div>
		</div>
		<MyTextarea
			v-model="sendResult"
			placeholder="POOLNFT INIT response will be displayed here"
			:readonly="true"
			:copyable="true"
			:deletable="false"
		/>
...
// 表单数据
const form = ref<PoolNftInitParams>({
	nft_contract_address: '',
	address: '',
...
const handleSubmit = async () => {
	...
	try {
		...
		const errorMsg =
			error instanceof Error ? error.message : 'Failed to send POOLNFT INIT transaction';
