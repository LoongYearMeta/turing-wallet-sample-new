	...
	<form class="msg-form" @submit.prevent="handleSubmit">
		<div class="msg-form-header">
			<div class="msg-form-title">
				...
				<span>POOLNFT LP Consume</span>
			</div>
			<p class="msg-form-description">Enter the LP consume parameters to redeem liquidity.</p>
		</div>
		...
		<div class="form-item-btn-container">
			<button class="form-button-submit" type="submit" :disabled="!isFormValid || isSubmitting">
				...
				<span v-else>Consume Liquidity</span>
			</button>
		</div>
	</form>
	...
	<div class="msg-form msg-result">
		<div class="msg-form-header">
			<div class="msg-form-title">
				...
				<span>POOLNFT LP Consume Result</span>
			</div>
		</div>
		<MyTextarea
			v-model="sendResult"
			placeholder="POOLNFT LP consume response will be displayed here"
			:readonly="true"
			:copyable="true"
			:deletable="false"
		/>
...
const form = ref<PoolNftConsumeForm>({
...
const handleSubmit = async () => {
	...
	try {
		...
		const errorMsg =
			error instanceof Error ? error.message : 'Failed to send POOLNFT LP Consume transaction';
