	...
	<form class="msg-form" @submit.prevent="handleSubmit">
		<div class="msg-form-header">
			<div class="msg-form-title">
				...
				<span>POOLNFT LP Increase</span>
			</div>
			<p class="msg-form-description">Provide the details to increase liquidity for the PoolNFT.</p>
		</div>
		...
		<div class="form-item">
			<label>Liquidity Address <span class="required">*</span></label>
			<MyTextarea
				v-model="form.address"
				placeholder="Please enter the liquidity recipient address"
				:readonly="false"
				:copyable="true"
				:deletable="true"
			/>
		...
		<div class="form-item-btn-container">
			<button class="form-button-submit" type="submit" :disabled="!isFormValid || isSubmitting">
				...
				<span v-else>Increase Liquidity</span>
			</button>
		</div>
	</form>
	...
	<div class="msg-form msg-result">
		<div class="msg-form-header">
			<div class="msg-form-title">
				...
				<span>POOLNFT LP Increase Result</span>
			</div>
		</div>
		<MyTextarea
			v-model="sendResult"
			placeholder="POOLNFT LP increase response will be displayed here"
			:readonly="true"
			:copyable="true"
			:deletable="false"
		/>
...
const form = ref<PoolNftIncreaseForm>({
...
const validateForm = () => {
	...
	if (!form.value.address.trim()) {
		errors.value.address = 'Liquidity address is required';
		isValid = false;
	} else {
...
const handleSubmit = async () => {
	...
	try {
		...
		toastApi.showSuccess('POOLNFT LP Increase transaction sent successfully', 3000);
		// 提交成功后清除缓存并重置表单（不清空结果）
		handleSubmitSuccess();
		await resetForm();
	} catch (error) {
		console.error('POOLNFT LP Increase transaction error:', error);
		const errorMsg =
			error instanceof Error ? error.message : 'Failed to send POOLNFT LP Increase transaction';
