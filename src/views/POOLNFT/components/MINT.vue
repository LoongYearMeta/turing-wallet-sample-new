<template>
	<form class="msg-form" @submit.prevent="handleSubmit">
		...
		<div class="form-item">
			...
			<input
				v-model="form.serverProvider_tag"
				type="text"
		...
		<div class="form-item">
			...
			/>
			<div v-if="errors.serviceFeeRate" class="form-item-error">{{ errors.serviceFeeRate }}</div>
		</div>

		<div class="form-item checkbox-item">
			<label class="checkbox-label">
				<input type="checkbox" v-model="form.with_lock" />
		...
		<div class="form-item checkbox-item">
			<label class="checkbox-label">
				<input type="checkbox" v-model="form.isLockTime" />
				<span>Enable lock time (`isLockTime`)</span>
			</label>
		</div>
		...
		<div class="form-item-btn-container">
			<button class="form-button-submit" type="submit" :disabled="!isFormValid || isSubmitting">
				...
				<span v-else>Send POOLNFT MINT</span>
			</button>
		</div>
	</form>
	...
	<div class="msg-form msg-result">
		<div class="msg-form-header">
			<div class="msg-form-title">
				...
				<span>POOLNFT MINT Result</span>
			</div>
		</div>
		<MyTextarea
			v-model="sendResult"
			placeholder="POOLNFT MINT response will be displayed here"
			:readonly="true"
			:copyable="true"
			:deletable="false"
		/>
...
const DEFAULT_SERVICE_FEE_RATE = '25';
const DEFAULT_LP_COST_AMOUNT = '5';
...
const validateForm = (): boolean => {
	...
	const pubKeys = parsePubKeyLock();
	if (form.value.with_lock && pubKeys.length === 0) {
...
const isFormValid = computed(() => {
	return (
		...
		!errors.value.lpCostAmount &&
		!errors.value.lpPlan &&
		(!form.value.with_lock || parsePubKeyLock().length > 0)
	);
});
...
const parsePubKeyLock = () => {
	return form.value.pubKeyLockInput
		.split('\n')
		.map((key) => key.trim())
...
const handleSubmit = async () => {
	...
	try {
		const params: any[] = [
			{
				...
				with_lock: form.value.with_lock,
				pubKeyLock: parsePubKeyLock(),
				lpPlan: Number(form.value.lpPlan),
				isLockTime: form.value.isLockTime,
			},
		];

		if (form.value.serverProvider_tag.trim()) {
			params[0].serverProvider_tag = form.value.serverProvider_tag.trim();
		}

		if (form.value.lpCostAddress.trim()) {
			params[0].lpCostAddress = form.value.lpCostAddress.trim();
		}

		if (form.value.lpCostAmount && validatePositiveNumber(form.value.lpCostAmount, true)) {
			params[0].lpCostAmount = Number(form.value.lpCostAmount);
		}
		...
		console.log('POOLNFT_MINT Generated Data:', params);
		...
		const errorMsg =
			error instanceof Error ? error.message : 'Failed to send POOLNFT MINT transaction';
