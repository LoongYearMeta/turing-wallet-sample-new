	...
	<div class="message-signing-page">
		...
		<div class="msg-form msg-result">
			...
			<div class="form-item-btn-container">
				...
				>
					<span v-if="isBalanceLoading">Fetching...</span>
					<span v-else>Get Balance</span>
				</button>
			</div>
			<MyTextarea
				v-model="balance"
				placeholder="Balance response will be displayed here"
				:readonly="true"
				:copyable="true"
				:deletable="true"
			/>
...
const ensureTuringSdk = (method: 'getPubKey' | 'getAddress' | 'getBalance') => {
...
const handleGetBalance = async () => {
	...
	try {
		const turingSdk = ensureTuringSdk('getBalance');
		const balanceResult = await turingSdk.getBalance();
		// 将余额结果格式化为 JSON 字符串以便显示
		balance.value = JSON.stringify(balanceResult, null, 2);
		toastApi.showSuccess('Balance fetched successfully', 3000);
	} catch (error) {
		console.error('Get Balance error:', error);
		toastApi.showError('Failed to fetch Balance', 3000);
		balance.value = '';
	} finally {
